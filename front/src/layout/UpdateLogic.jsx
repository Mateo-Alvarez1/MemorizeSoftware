...
export const UpdateLogic = ({ modal , setModal , id  }) => {
 
  const [show, setShow] = useState(false);
  const token = localStorage.getItem('token');
  const cleanToken = JSON.parse(token);
  const { onHandleInput, onHandleSubmit } = UseUpdate(
    id , 
    'register',
    {
      name: '',
       ...
       <div className='flex items-center justify-between w-full'>
        ...
        >
          ...
          <div className="flex flex-col my-3  relative">
            ...
            />
            {
              show ? <img onClick={ () => setShow(!show)} className='top-8 left-52 w-6 absolute' src={eyeOpen} alt="" />
              : <img onClick={ () => setShow(!show)} className='top-8 left-52 w-6 absolute' src={eyeClose} alt="" />
            }
          </div>
